...
const TripStylesMobile = ({setIsActiveParent}) => {
            ...
            <div className="tabs__controls row x-gap-40 y-gap-10 mb-20 lg:x-gap-20 pb-30 js-tabs-controls">
                {itemList.map((item, i) => (
                    <div className="col-auto" key={i} onClick={()=>setActiveNestedMenu(i)}>
                        <button className={`tabs__button text-light-1 fw-600 text-16 js-tabs-button ${activeNestedMenu == i ? 'nested-menu-active':'nested-menu-inactive'} `}>
                            {item}
                        </button>
                    </div>
                ))}
...
export default TripStylesMobile;
