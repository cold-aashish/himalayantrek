...
import { categorieMegaMenuItemsTibet } from "../../data/mainMenuData";
import {
    isActiveParent,
    isActiveLink,
...
const TourPackages = ({setIsActiveParent}) => {
    ...
    useEffect(() => {
        ...
        categorieMegaMenuItemsTibet.map((megaMenu=>{
    ...
    return (
        <div className="tabs -underline-2 js-tabs">
            <div className="tabs__controls row x-gap-40 y-gap-10 lg:x-gap-20 pb-30 js-tabs-controls">
                {itemList.map((item, i) => (
                    <div className="col-auto" key={i} onClick={()=>setActiveNestedMenu(i)}>
                        <button className={`tabs__button text-light-1 text-16 fw-600 js-tabs-button ${activeNestedMenu == i ? 'nested-menu-active':'nested-menu-inactive'} `}>
                            {item}
                        </button>
                    </div>
                ))}
            ...
            <div className="tabs__content js-tabs-content bg-blue-3 p-2">

                <div className={'react-tabs__tab-panel--selected'}>
                    {categorieMegaMenuItemsTibet[activeNestedMenu]?.menuCol?.map((megaCol, i) => (
                                    ...
                                    <div className="mega__item" key={item.id}>
                                        ...
                                        <div className="y-gap-5 text-15 pt-5">
                                            {item?.menuList?.map((list, i) => (
                                                ...
                                                >
                                                    <Link href={list.routePath} className="text-14">{list.name}</Link>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
