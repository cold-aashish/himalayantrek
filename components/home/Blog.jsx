      ...
      {blogs.slice(0, 2).map((item) => (
        <div className="col-lg-4 col-sm-6" key={item.id}>
          <Link
            ...
            href={`/`}
            className="blogCard -type-1 d-block  custom-card-type-3 "
          >
            ...
            <div className="mt-10">
              <h4 className="text-dark-1 text-25 fw-600">{item.title}</h4>
              <div className="text-light-1 text-16 lh-12 mt-5">{item.date}</div>
            </div>
          </Link>
        </div>
      ))}
